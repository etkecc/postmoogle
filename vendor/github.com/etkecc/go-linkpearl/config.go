...
type Config struct {
	// Homeserver url
	Homeserver string
	// Login is localpart for password auth or full mxid for shared secret auth (honoroit vs @honoroit:example.com).
	Login string
	...
	// JoinPermit is a callback that tells if linkpearl should respond to the given "invite" event and join the room.
